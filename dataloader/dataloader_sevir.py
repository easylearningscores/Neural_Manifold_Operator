...
class Sevir_ir069(Dataset):
    def __init__(self, data_path, transform=None):
        ...
        self.data = torch.tensor((self.data - self.mean) / self.std, dtype=torch.float32)
    ...
    def __getitem__(self, idx):
        input_frames = self.data[idx][:13]
        output_frames = self.data[idx][13:25]
        return input_frames, output_frames


def load_data(batch_size, val_batch_size, data_root, num_workers):
    dataset = Sevir_ir069(data_path=data_root + 'SEVIR_ir069.npy', transform=None)
