...
class diff_react(Dataset):
    def __init__(self, data_path, transform=None):
        self.data = torch.from_numpy(np.load(data_path))[:, :-1, :, :, :]
...
def load_data(batch_size, val_batch_size, data_root, num_workers):
    dataset = diff_react(data_path=data_root + 're_diff_react.npy', transform=None)
...
if __name__ == "__main__":
    data_path = '/data/workspace/yancheng/MM/zD-diff-react/'
